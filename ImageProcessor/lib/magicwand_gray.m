function bin_mask = magicwand_gray(im, ylist, xlist, tolerance)
...
% Modified by Zoom Nguyen, dhnguyen at princeton dot edu
...
if ndims(im) ~=2
    error('Input image must be in grayscale format');
end
...
for idx = 1:length(ylist)
    ref = double(im(ylist(idx), xlist(idx), 1));
    color_mask = color_mask | (im - ref) .^ 2 <= tolerance ^ 2;
end
